...
router.delete('/:clanname', passport.authenticate('jwt', { session: false, failureRedirect: '/api/errors/unauthorized' }), (req, res) => {
  let clan;

  // Find the clan
  Clan.findOne({ name: req.params.clanname }, (err, foundClan) => {
  ...
  if (req.user._id === clan.creator_id) {
    ...
    User.find({ clan_id: clan._id }, (err, users) => {
      if (err) {
        res.send({ success: false, message: err });
      } else {
...
router.post('/:clanname', passport.authenticate('jwt', { session: false, failureRedirect: '/api/errors/unauthorized' }), (req, res) => {
  const clan = new Clan({
...
router.get('/:clanname/join', passport.authenticate('jwt', { session: false, failureRedirect: '/api/errors/unauthorized' }), (req, res) => {
  let clan;
...
router.get('/:clanname/leave', passport.authenticate('jwt', { session: false, failureRedirect: '/api/errors/unauthorized' }), (req, res) => {
  let clan;
...
router.post('/:clanname/requests/:username', passport.authenticate('jwt', { session: false, failureRedirect: '/api/errors/unauthorized' }), (req, res) => {
...
router.delete('/:clanname/requests/:username', passport.authenticate('jwt', { session: false, failureRedirect: '/api/errors/unauthorized' }), (req, res) => {
  if (req.user.username != req.params.username) {
...
router.get('/:clanname/requests/:username/accept', passport.authenticate('jwt', { session: false, failureRedirect: '/api/errors/unauthorized' }), function (req, res) {
  ...
  if (clan.open) {
    ...
    User.findOne({ name: req.params.username }, (err, foundUser) => {
      if (err) { // If there's an error, return it.
        res.send({ success: false, message: err });
      } else if (foundClan) { // If there's a clan, store it.
    ...
    if (clan.requestedUser_ids.indexOf(acceptingUser._id) > -1 && req.user.clanRequest_ids.indexOf(clan._id) > -1) {
      // Remove all clan references from member users...
      acceptingUser.clanRequest_ids.forEach((id) => {
        Clan.findById(id, (err, foundClan) => {
...
router.get('/:clanname/requests/:username/deny', passport.authenticate('jwt', { session: false, failureRedirect: '/api/errors/unauthorized' }), (req, res) => {
  let clan;
...
router.get('/:clanname/invite/:username', passport.authenticate('jwt', { session: false, failureRedirect: '/api/errors/unauthorized' }), (req, res) => {
  let clan;
